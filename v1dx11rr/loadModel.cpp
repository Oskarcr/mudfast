...
bool CObjParser::LoadFile(char* szFileName)
{
	...
	while (!feof(fp))
	{
		...
		if (strcmp(buf, "f") == 0) {
			char line[BUFSIZE];
			if (!fgets(line, BUFSIZE, fp)) break;
			...
			while (token && vertCount < 4) {
				...
				if (strstr(token, "//") == NULL)
					sscanf_s(token, "%d/%d/%d", &a, &b, &c);
				else
					sscanf_s(token, "%d//%d", &a, &c);
			...
			if (vertCount == 3) {
				for (int i = 0; i < 3; i++) {
					AddIntBuffer(verts[i], &fbuf, &fbufsize, &fbufpos);
					AddIntBuffer(uvs[i], &fbuf, &fbufsize, &fbufpos);
					AddIntBuffer(norms[i], &fbuf, &fbufsize, &fbufpos);
				}
			}
			else if (vertCount == 4) {
				...
				for (int t = 0; t < 2; t++) {
					for (int i = 0; i < 3; i++) {
						int idx = tri[t][i];
						AddIntBuffer(verts[idx], &fbuf, &fbufsize, &fbufpos);
						AddIntBuffer(uvs[idx], &fbuf, &fbufsize, &fbufpos);
						AddIntBuffer(norms[idx], &fbuf, &fbufsize, &fbufpos);
					}
				}
			}
