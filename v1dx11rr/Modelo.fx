...
float4 PS_Main(PS_Input pix) : SV_TARGET
{
	...
	if (lightIntensity > 0) {
		...
		color += (float4(1.0f, 1.f, 1.f, 1.0f) * lightIntensity);
...
//dia y noche
cbuffer cbLighting : register(b5)
{
    float lightFactor;
}
...
float4 PS_Main(PS_Input pix) : SV_TARGET
{
	//modificado el anterior para poder hacer la iluminacion de los modelos 
    ...
    float3 ambient = float3(0.2f, 0.2f, 0.25f) * lightFactor;
    float3 diffuse = float3(1.0f, 1.0f, 1.0f) * lightIntensity * lightFactor;
    float3 color = ambient + diffuse;
    float3 reflection = normalize(2 * lightIntensity * pix.normal - lightDir);
    float specular = pow(saturate(dot(reflection, pix.campos)), pix.specForce);
    float3 finalSpec = specular * specularMap.rgb;
