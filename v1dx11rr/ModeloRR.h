...
public:
	...
	bool CargaParametros(char* ModelPath, WCHAR* diffuseTex, WCHAR* specularTex)
	{
		...
		desc.ByteWidth = sizeof(LightingBuffer);
		desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
		d3dResult = d3dDevice->CreateBuffer(&desc, nullptr, &lightingBuffer);
		if (FAILED(d3dResult))
		{
			return false;
		}
	...
	void UnloadContent() // no manches era esto xdd (tenia bool)
	{
		if (colorMapSampler)
		...
		char angle, float scale, float luzFactor) //factor de luz para día y noche LUZFACOR
	{
		...
		//día y noche
		LightingBuffer data;
		data.lightFactor = luzFactor;
		data.padding = D3DXVECTOR3(0, 0, 0);
		d3dContext->UpdateSubresource(lightingBuffer, 0, nullptr, &data, 0, 0);
		d3dContext->PSSetConstantBuffers(5, 1, &lightingBuffer);
		///
