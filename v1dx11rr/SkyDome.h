...
// AQUI EN PUBLIC: METO LO DEL CICLO DIA Y NOCHE
...
public:
	...
	// AGREGANDO EL CICLO DiA Y NOCHE
	void setTexture(WCHAR* ruta, float lightFactor)
	{
		if (textura) textura->Release();
		D3DX11CreateShaderResourceViewFromFile((*d3dDevice), ruta, 0, 0, &textura, 0);

		// para actualizar la iluminacion del cielo
		matrices->valores = D3DXVECTOR4(lightFactor, 0.0f, 0.0f, 0.0f);
...
public:
	SkyDome(int slices, int stacks, float radio, ID3D11Device** d3dDevice, ID3D11DeviceContext** d3dContext, WCHAR* diffuseTex)
	{
		this->slices = slices;
		this->stacks = stacks;
		this->radio = radio;
	...
	void Render(D3DXVECTOR3 trans)
	{
		...
		D3DXMATRIX worldMat;
		D3DXMatrixRotationY(&rotMat, rotacion);
