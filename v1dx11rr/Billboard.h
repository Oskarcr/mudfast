...
public:
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, D3DXVECTOR3 poscam, float xx, float zz, float posy, float escala, vector2 uv1, vector2 uv2, vector2 uv3, vector2 uv4)
	{
		posx = xx;
		posz = zz;
		vertices = new VertexComponent[4];
		...
		vertices[0].pos.y = 0;
		vertices[0].pos.z = -1 * escala;
		vertices[0].UV.x = uv1.u;
		vertices[0].UV.y = uv1.v;

		vertices[1].pos.x = 0;
		vertices[1].pos.y = 2 * escala;
		vertices[1].pos.z = -1 * escala;
		vertices[1].UV.x = uv2.u;
		vertices[1].UV.y = uv2.v;

		vertices[2].pos.x = 0;
		vertices[2].pos.y = 2 * escala;
		vertices[2].pos.z = 1 * escala;
		vertices[2].UV.x = uv3.u;
		vertices[2].UV.y = uv3.v;

		vertices[3].pos.x = 0;
		vertices[3].pos.y = 0;
		vertices[3].pos.z = 1 * escala;
		vertices[3].UV.x = uv4.u;
		vertices[3].UV.y = uv4.v;
